...
const MenuBtnsComponent: React.FC<Props> = ({ onClick }) => {
	...
	const { locale: storedLocale } = useSelector(
	...
	useEffect(() => {
		if (storedLocale) setLocaleFromStorage(storedLocale);
		...
		<div className=" relative md:flex justify-start items-center w-full h-full">
			...
			<Link to="/">
				...
				>
					...
					<span className="md:p-1 uppercase text-sm">{home}</span>
				</div>
			</Link>
			...
			<div onClick={handleOnOnToggleLocale} className={MenuClassName}>
			...
			<div className="absolute md:p-1 right-0">
				<div className="flex md:flex-row flex-col gap-1 justify-end items-end">
					<div
						className="flex justify-end items-end p-1 border-secondary-col right-0 hover:bg-red-400 hover:bg-opacity-45 hover:scale-105 rounded-md"
					...
					>
						...
						<span className="uppercase text-sm text-secondary-col">
							{isAuthenticated ? logOutTitle : login}
						</span>
					</div>
				</div>
			</div>
